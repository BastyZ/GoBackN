...
class SendWindow:
    ...
    def has_sent_first_package(self):
        return self.first_package_flag
    ...
    def __update_timeout_interval(self, sample_rtt):
        if self.first_package_flag:
            self.first_package_flag = False
    ...
    def load_next(self):
        ...
        if len(self.window) < self.window_size and self.package_index < len(self.packages):
            ...
            self.window.append(
                [
                    str(self.package_index % 10**self.sequence_digits).zfill(self.sequence_digits),                    # Sequence number of last on window
    ...
    def ack(self, ack_seq_num, checksum):
        ...
        if first < last and (ack_lt_fst or ack_gt_lst):
            ...
            print("ReceiverThread :: Received ACK N° {} | checksum {}".format(ack_seq_num, checksum))
    ...
    def advance(self, seq_num):
        if seq_num != self.window[0][0]:
            print("Window :: Advancing from {} to {}".format(self.window[0][0], seq_num))
        while len(self.window) > 0 and int(seq_num) + 1 != int(self.window[0][0]):
